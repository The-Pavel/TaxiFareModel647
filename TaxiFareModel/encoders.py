...
class DistanceToCenter(BaseEstimator, TransformerMixin):
    def __init__(self,
        ...
        self.end_lat = end_lat
        self.end_lon = end_lon
        self.nyc_center = (40.7141667, -74.0063889)
    ...
    def transform(self, X, y=None):
        assert isinstance(X, pd.DataFrame)
        X_ = X.copy()
        X_["nyc_lat"], X_["nyc_lon"] = self.nyc_center[0], self.nyc_center[1]
        X_["pickup_to_center"] = haversine_vectorized(
            X_,
            start_lat=self.start_lat,
            start_lon=self.start_lon,
            end_lat='nyc_lat',
        ...
        X_["dropoff_to_center"] = haversine_vectorized(
            X_,
            start_lat=self.end_lat,
            start_lon=self.end_lon,
            end_lat='nyc_lat',
            end_lon='nyc_lon'
        )
        return X_[['pickup_to_center', "dropoff_to_center"]]
